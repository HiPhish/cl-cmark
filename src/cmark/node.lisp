...
(defclass document-node (parent-node)
  ()
  (:documentation "Root node of the entire document."))

(defclass block-quote-node (parent-node child-node)
  ()
  (:documentation "Represents a block quotation."))

(defclass list-node (parent-node child-node)
...
(defclass item-node (parent-node child-node)
  ()
  (:documentation "Represents an item inside a list (bullet or unordered)."))

(defclass code-block-node (child-node)
...
(defclass html-block-node (child-node)
  ((literal :initarg :literal :accessor node-literal :type (or string null)
            :documentation "Text of the node (might be empty)."))
  (:documentation "Block-level embedded HTML code."))

(defclass custom-block-node (parent-node child-node)
...
(defclass paragraph-node (parent-node child-node)
  ()
  (:documentation "A paragraph of the document."))

(defclass heading-node (parent-node child-node)
...
(defclass thematic-break-node (child-node)
  ()
...
(defclass emph-node (parent-node child-node)
  ()
  (:documentation "Container for emphatic text."))

(defclass strong-node (parent-node child-node)
  ()
  (:documentation "Container for strong text."))

(defclass link-node (parent-node child-node)
...
(defclass image-node (parent-node child-node)
  ((url :initarg :url :accessor node-url :type string
